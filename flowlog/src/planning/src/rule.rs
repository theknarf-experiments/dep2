...
impl RuleQueryPlan {
    ...
    fn recursive_antijoins(
        catalog: &Catalog,
        planning_atom_signatures: &[AtomSignature],
        last_transformation: Transformation,
        negated_atom_signatures: &[AtomSignature],
        head_key_arguments: &[String],
        head_value_arguments: &[String],
        ...
        if negated_atom_signatures.is_empty() {
            ...
            let (left_transformation, mut tree) = Self::recursive_antijoins(
                ...
                last_transformation,
                &negated_atom_signatures[..negated_atom_signatures.len() - 1],
                &negated_head_key_arguments,
    ...
    fn recursive_semijoins(
        catalog: &Catalog,
        planning_atom_signature: &AtomSignature,
        subatom_signatures: &[AtomSignature],
        head_key_arguments: &[String],
        head_value_arguments: &[String],
        active_comparison_predicates: &[usize],
    ) -> (
        ...
        if subatom_signatures.is_empty() {
            ...
            let leaf_transformation = Transformation::kv_to_kv(
                ...
                &catalog.top_down_trace(head_key_arguments, &[*planning_atom_signature]),
                &catalog.top_down_trace(head_value_arguments, &[*planning_atom_signature]),
            ...
            let (left_transformation, mut tree) = Self::recursive_semijoins(
                catalog,
                planning_atom_signature,
                &subatom_signatures[..subatom_signatures.len() - 1],
                &sub_head_key_arguments,
            ...
            let root_transformation = Transformation::join(
                ...
                &catalog.top_down_trace(head_key_arguments, &[*planning_atom_signature]), // the search scope for the head arguments should only from the planning atom as it is a semijoin
                &catalog.top_down_trace(head_value_arguments, &[*planning_atom_signature]),
                &[], // no comparison expressions at the semijoin level
            );
