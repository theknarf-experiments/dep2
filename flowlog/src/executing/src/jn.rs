...
/* -------------------------------------------------------------------------------------------------------------------- */
fn jn_deconstructor<const N: usize>(args: &Arc<Vec<TransformationArgument>>) -> ArrayVec<(bool, bool, usize), N> {
...
#[inline(always)]
fn jn_extractor<const K: usize, const V: usize, const W: usize, const N: usize>(k: &Row<K>, v1: &Row<V>, v2: &Row<W>, extracts: &[(bool, bool, usize)]) -> Row<N> {
...
/*                  → (k, ∅) */
/*                  → (∅, v) */
pub fn jn_logic<const K: usize, const V: usize, const W: usize, const N: usize>(flow: &TransformationFlow) -> 
...
{   
    let rids = if let TransformationFlow::JnToKV { key, value, .. } = flow {
        ...
        } else {  panic!("jn row: must be a jn flow"); };
...
/* -------------------------------------------------------------------------------------------------------------------- */
fn cartesian_deconstructor<const N: usize>(args: &Arc<Vec<TransformationArgument>>) -> ArrayVec<(bool, usize), N> {
    args.iter().filter_map(|arg| match arg {
...
#[inline(always)]
fn cartesian_extractor<const V: usize, const W: usize, const N: usize>(v1: &Row<V>, v2: &Row<W>, extracts: &[(bool, usize)]) -> Row<N> {
    let mut row = Row::<N>::new();
...
pub fn cartesian_logic<const V: usize, const W: usize, const N: usize>(flow: &TransformationFlow) -> 
    impl FnMut(&(), &Row<V>, &Row<W>) -> Option<Row<N>> 
{   
    let rids = if let TransformationFlow::JnToKV { key, value, .. } = flow {
            cartesian_deconstructor::<N>(&Arc::new(key.iter().chain(value.iter()).cloned().collect()))
        } else {  panic!("cartesian: must be a jn flow"); };
    let compares = flow.compares().clone();
...
/* -------------------------------------------------------------------------------------------------------------------- */
fn v1_jn_deconstructor<const N: usize>(args: &Arc<Vec<TransformationArgument>>) -> ArrayVec<(bool, usize), N> {
    args.iter().filter_map(|arg| match arg {
...
#[inline(always)]
fn v1_jn_extractor<const K: usize, const V: usize, const N: usize>(k: &Row<K>, v1: &Row<V>, extracts: &[(bool, usize)]) -> Row<N> { // v1 -- always from left
    let mut row = Row::<N>::new();
...
/*                  → (k, ∅) */
/*                  → (∅, v) */
pub fn v1_jn_logic<const K: usize, const V: usize, const N: usize>(flow: &TransformationFlow) -> impl FnMut(&Row<K>, &Row<V>, &()) -> Option<Row<N>> {
    let rids = 
        if let TransformationFlow::JnToKV { key, value, .. } = flow {
...
/* renders for (k, ∅) jn (k, ∅) */
/* -------------------------------------------------------------------------------------------------------------------- */
fn v2_jn_deconstructor<const N: usize>(args: &Arc<Vec<TransformationArgument>>) -> ArrayVec<usize, N> {
    args.iter().filter_map(|arg| match arg {
...
fn v2_jn_extractor<const K: usize, const N: usize>(k: &Row<K>, extracts: &[usize]) -> Row<N> { 
    let mut row = Row::<N>::new();
    for &id in extracts { row.push(k.column(id)); }
    row
}
...
/*                  → (k, ∅) */
/*                  → (∅, v) */
pub fn v2_jn_logic<const K: usize, const N: usize>(flow: &TransformationFlow) -> impl FnMut(&Row<K>, &(), &()) -> Option<Row<N>> {
    let rids =
        if let TransformationFlow::JnToKV { key, value, .. } = flow {
...
pub fn aj_flatten<const K: usize, const V: usize, const N: usize>(flow: &TransformationFlow) -> impl FnMut(&Row<K>, &Row<V>) -> Row<N> {
    let rids = 
        if let TransformationFlow::JnToKV { key, value, .. } = flow {
            v1_jn_deconstructor::<N>(&Arc::new(key.iter().chain(value.iter()).cloned().collect()))
        } else {
...
pub fn v1_aj_flatten<const K: usize, const N: usize>(flow: &TransformationFlow) -> impl FnMut(&Row<K>, &()) -> Row<N> {
    let rids = 
        if let TransformationFlow::JnToKV { key, value, .. } = flow {
            v2_jn_deconstructor::<N>(&Arc::new(key.iter().chain(value.iter()).cloned().collect()))
        } else {
...
#[inline(always)]
fn jn_extractor_fat(k: &FatRow, v1: &FatRow, v2: &FatRow, extracts: &[(bool, bool, usize)]) -> FatRow {
...
pub fn jn_logic_fat(flow: &TransformationFlow) -> impl FnMut(&FatRow, &FatRow, &FatRow) -> Option<FatRow> {
    let rids = if let TransformationFlow::JnToKV { key, value, .. } = flow {
            jn_deconstructor_fat(&Arc::new(key.iter().chain(value.iter()).cloned().collect()))
        } else {  panic!("jn_logic_fat: must be a jn flow"); };
    let compares = flow.compares().clone();
...
pub fn cartesian_logic_fat(flow: &TransformationFlow) -> impl FnMut(&(), &FatRow, &FatRow) -> Option<FatRow> {
    let rids = if let TransformationFlow::JnToKV { key, value, .. } = flow {
            cartesian_deconstructor_fat(&Arc::new(key.iter().chain(value.iter()).cloned().collect()))
        } else {  panic!("cartesian_logic_fat: must be a jn flow"); };
    let compares = flow.compares().clone();
...
pub fn v1_jn_logic_fat(flow: &TransformationFlow) -> impl FnMut(&FatRow, &FatRow, &()) -> Option<FatRow> {
    let rids = if let TransformationFlow::JnToKV { key, value, .. } = flow {
...
pub fn v2_jn_logic_fat(flow: &TransformationFlow) -> impl FnMut(&FatRow, &(), &()) -> Option<FatRow> {
    let rids = if let TransformationFlow::JnToKV { key, value, .. } = flow {
