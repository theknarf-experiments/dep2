...
impl TransformationFlow {
    ...
    pub fn compares(&self) -> &[ComparisonExprArgument] {
        match self {
    ...
    pub fn kv_argument_flow_map(
        ...
        key_signatures
            .iter()
            .enumerate()
            .map(|(id, signature)| (*signature, TransformationArgument::KV((false, id))))
            .chain(
                value_signatures
                    .iter()
                    .enumerate()
                    .map(|(id, signature)| (*signature, TransformationArgument::KV((true, id)))),
            )
            .collect()
    }

    pub fn kv_to_kv(
        input_key_signatures: &[AtomArgumentSignature],
        input_value_signatures: &[AtomArgumentSignature],
        output_key_signatures: &[AtomArgumentSignature],
        output_value_signatures: &[AtomArgumentSignature],
        const_eq_constraints: &[(AtomArgumentSignature, Const)],
        ...
        let var_signatures: Vec<AtomArgumentSignature> = var_eq_constraints
            .iter()
            .map(|(left_signature, _)| *left_signature)
            .collect();

        let alias_signatures: Vec<AtomArgumentSignature> = var_eq_constraints
            .iter()
            .map(|(_, right_signature)| *right_signature)
            .collect();
        ...
        let flow_compare_signatures = compare_exprs
            .iter()
            .map(|comp| {
                let left_signatures: Vec<AtomArgumentSignature> =
                    comp.left().signatures().into_iter().copied().collect();
                let right_signatures: Vec<AtomArgumentSignature> =
                    comp.right().signatures().into_iter().copied().collect();
    ...
    pub fn join_to_kv(
        input_left_key_signatures: &[AtomArgumentSignature],
        input_left_value_signatures: &[AtomArgumentSignature],
        _input_right_key_signatures: &[AtomArgumentSignature], // not necessary
        input_right_value_signatures: &[AtomArgumentSignature],
        output_key_signatures: &[AtomArgumentSignature],
        output_value_signatures: &[AtomArgumentSignature],
        compare_exprs: &[ComparisonExprPos],
    ) -> Self {
        ...
        let right_signature_map = Self::kv_argument_flow_map(&[], input_right_value_signatures) // Self::kv_argument_flow_map(input_right_key_signatures, input_right_value_signatures)
            .into_iter()
            .map(|(signature, trace)| {
                let join_trace = match trace {
                    TransformationArgument::KV((key_or_value, id)) => {
